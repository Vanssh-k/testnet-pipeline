...
], validate, GatewayController.add_subdomain);

router.get("/check_subdomain", [
  query('subDomain').not().isEmpty().withMessage("subDomain not found"),
], validate, GatewayController.check_subdomain);

router.get("/get_subdomain", [
  query('publicKey').not().isEmpty().withMessage("publicKey not found"),
], validate, GatewayController.get_subdomain);
