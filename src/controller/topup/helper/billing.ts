...
import billingABI from '../../../contract_abi/billing.js'

const polygonProvider = new ethers.JsonRpcProvider(envConfig.polygon_rpc)
const LighthouseBillingContract = new ethers.Contract(envConfig.lighthouse_billing_address, billingABI, polygonProvider)

const getSubscriptionStatus = async (
  publicKey: string,
  subId: number,
): Promise<{ status: boolean; subscriptionId: number }> => {
