...
test('CID TAG Test', async () => {
  ...
  const createTagRes = JSON.parse(
    (await supertest(app).post('/api/user/create_tag').send(data).set('Authorization', `Bearer ${apiKey}`)).text,
  )
  ...
  const getTagDetails = JSON.parse(
    (await supertest(app).get(`/api/user/get_tag_details?tag=${tag}`).set('Authorization', `Bearer ${apiKey}`)).text,
  )
  expect(typeof getTagDetails.data.tag).toBe('string')

  // Get All CID
  const allTags = JSON.parse(
    (await supertest(app).get('/api/user/get_all_tags').set('Authorization', `Bearer ${apiKey}`)).text,
  )
  expect(typeof allTags.data[0]['tag']).toBe('string')

  // Remove Key
  const removeTagRes = JSON.parse(
    (await supertest(app).delete(`/api/user/remove_tag?tag=${tag}`).set('Authorization', `Bearer ${apiKey}`)).text,
  )
