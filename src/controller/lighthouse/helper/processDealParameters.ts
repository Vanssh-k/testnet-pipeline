...
const processDealParam = (dealParam: DealParameters | null, id: string): ProcessedDealParameters | null => {
  try {
    ...
    if (dealParam.repair_threshold === null) {
      ...
      dealData.repair_threshold = Math.max(dealParam.repair_threshold, 240)
        ? Math.max(dealParam.repair_threshold, 240)
        : 240
    }
    ...
    if (dealParam.renew_threshold === null) {
      ...
      dealData.renew_threshold = Math.max(dealParam.renew_threshold, 240)
        ? Math.max(dealParam.renew_threshold, 240)
        : 240
    }
    ...
    if (dealParam.add_mock_data === null) {
      ...
      dealData.add_mock_data = Math.min(Math.max(dealParam.add_mock_data, 0), 1024)
        ? (dealData.add_mock_data = Math.min(Math.max(dealParam.add_mock_data, 0), 1024))
        : null
    }
